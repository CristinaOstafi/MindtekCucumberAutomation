...
public class WebOrdersSteps {
   ...
   public void user_is_successfully_logged_in_and_lands_on_the_homepage() {
    ...
    public void user_validates_error_message(String invalidPassword) {
        Assert.assertEquals(invalidPassword,webOrdersLoginPage.errorMessage.getText());
    ...
    @When("User selects {string} and quantity {int}")
    public void user_selects_and_quantity(String product, Integer quantity) {
    ...
    public void user_validates_the_price_is_correctly_calculated_for(Integer quantity) {
        ...
        int expectedTotal;
        if(quantity>=10){
            expectedTotal = quantity*priceInt*(100-discount)/100;
            //expectedTotal = quantity*priceInt*discount/100;
    ...
    public void user_validates_created_order_is_in_the_list_of_all_orders() {
        webOrdersHomePage.viewAllOrdesrsTab.click();
